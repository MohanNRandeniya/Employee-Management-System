...
namespace EmployeeM
{
    public partial class ViewAbsent : Form
    {
        ...
        {
            conn = new conDB();
            SqlDataReader rdr = conn.dataReader("select * from Absent ");
        ...
        private void pbSearchAll_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    conn = new conDB();
                    SqlDataReader rdr = conn.dataReader("select * from Absent where FullName like '%" + txtSearchAll.Text + "%' ");
                    BindingSource bs = new BindingSource();
                    bs.DataSource = rdr;
                    bunifuCustomDataGrid1.DataSource = bs;
        ...
        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                conn = new conDB();
                SqlDataReader rdr = conn.dataReader("select * from Absent where Date = '" + dateTimePicker1.Value + "' ");
                BindingSource bs = new BindingSource();
                bs.DataSource = rdr;
                bunifuCustomDataGrid1.DataSource = bs;
